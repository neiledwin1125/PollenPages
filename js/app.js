...
async function connectPollen() {
  ...
  window.location.href = `https://enter.pollinations.ai/authorize?redirect_url=${redirect}&models=flux,openai&budget=100&expiry=30`;
}
...
function getImageUrl(prompt, imageModel, dimensions) {
  const [width, height] = dimensions.split('x');
  const enhancedPrompt = encodeURIComponent(prompt + " masterpiece, high quality, trending on artstation");
...
async function generateBook() {
  ...
  try {
    ...
    for (let i = 0; i < pages.length; i++) {
      ...
      pages[i].imageUrl = getImageUrl(pages[i].illustrationPrompt, imageModel, dimensions);
